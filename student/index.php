...
$qry=mysqli_query($conn,"select * ,concat(firstname,' ',lastname)as name from student_list where email='$email' ");
$users=mysqli_fetch_assoc($qry);

$eid=$users["email"];


$sql=mysqli_query($conn,"select * from feedback where student_id='$eid'");
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-3 col-md-2 sidebar" style="background-color:#0a225c;">
          <ul class="nav nav-sidebar">
			...
			<?php 
			$q=mysqli_query($conn,"select image from student_list where email='".$_SESSION['email']."'");
      ...
      <h2 style="margin:50px;" class="text-success"> Number of Feedback Provided : <?php echo($rows); ?> </h2><?php
